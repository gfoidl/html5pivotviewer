...
//    Copyright (C) 2013 Foidl Günther
//
...
PivotViewer.Views.TableView = PivotViewer.Views.IPivotViewerView.subClass({
	...
	Setup: function (width, height, offsetX, offsetY, tileMaxRatio) {
		...
		$('.pv-viewpanel').append("<div style='visibility: hidden; position: relative;' id='pv-table-loader'><div class='pv-image-loading'></div></div>");
	...
	GetButtonImage: function () {
		return PivotViewer.Utils.ImageUrl(baseUrl + "Content/css/pivotviewer/images/TableView.png");
	},
	GetButtonImageSelected: function () {
		return PivotViewer.Utils.ImageUrl(baseUrl + "Content/css/pivotviewer/images/TableViewSelected.png");
	},
	...
	CreateTable: function (currentFilter, selectedFacet, sortKey, sortReverse) {
		...
		if (sortReverse) {
			sortImage = PivotViewer.Utils.ImageUrl(baseUrl + "Content/css/pivotviewer/images/sort-up.png");
			//offset = +40;
		} else {
			sortImage = PivotViewer.Utils.ImageUrl(baseUrl + "Content/css/pivotviewer/images/sort-down.png");
			//offset = -40;
		}
		...
		var imgUrl = PivotViewer.Utils.ImageUrl(baseUrl + "Content/css/pivotviewer/images/goout.gif");
							...
							for (l = 0; l < this.tiles[j].facetItem.Facets[k].FacetValues.length; l++) {
								...
								if (this.IsFilterVisible(attribute)) {
									...
									if (this.tiles[j].facetItem.Href) {
										// Value is uri
										if (this.IsUri(value))
											tableRows.push({ key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven + "'><td id='pv-key' class='tooltipinter' title='Toggle item selection' style='color:#009933;cursor:pointer'>" + entity + " " + "<a href=" + this.tiles[j].facetItem.Href.replace(/'/g, "%27") + " target=\"_blank\"><img style='cursor:default;' id='pv-linkout' title='Follow the link' src='" + imgUrl + "' /></a></td><td id='pv-facet' class='tooltipcustom'  title='Show only this relation' style='color:#009933;cursor:pointer'>" + attribute + "</td><td id='pv-value' class='tooltipinter' title='Filter on this value' style='color:#009933;cursor:pointer'>" + value + " " + "<a href=" + value + " target=\"_blank\"><img style='cursor:default;' id=pv-linkout' title='Follow the link' src='" + imgUrl + "' /></a></td></tr>" });
										else
